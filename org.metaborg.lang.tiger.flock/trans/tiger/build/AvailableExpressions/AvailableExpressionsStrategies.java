...
public class FlowAnalysisStrategies {
  public static class get_expressions_0_0 extends Strategy {
    ...
    @Override public IStrategoTerm invoke(Context context, IStrategoTerm current) {
                                                                                    ...
                                                                                    IStrategoList result = factory. makeList(((Map<IStrategoTerm, IStrategoTerm> ) node. getProperty("expressions").lattice. value( )). entrySet( ). stream( ). map(n -> factory. makeList(Helpers. toTerm(n. getKey( )), Helpers. toTerm(n. getValue( )))). collect(Collectors. toList( )));
                                                                                    return result;
                                                                                  }
  }
}