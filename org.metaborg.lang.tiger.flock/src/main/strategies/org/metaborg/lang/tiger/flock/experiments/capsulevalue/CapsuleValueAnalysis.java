package org.metaborg.lang.tiger.flock.experiments.capsulevalue;
...
import org.metaborg.lang.tiger.flock.experiments.capsulevalue.CapsuleValueAnalysisProperties.ConstProp;
...
public class CapsuleValueAnalysis extends SingleAnalysis {
	public CapsuleValueAnalysis(int id) {
...
class TransferFunction5 extends TransferFunction {
	...
	public boolean eval(SingleAnalysis.Direction direction, FlockLattice res, Node node) {
		...
		Transient x = ((io.usethesource.capsule.Map.Immutable) ((FlockLattice) UserFunctions.values_f(prev)).value())
				.asTransient();
		x.__put(usrn, new Value(new ConstProp(factory.makeAppl(factory.makeConstructor("Const", 1),
				new IStrategoTerm[] { Helpers.toTerm(usri) }, null))));
		return TransferFunction.assignEvalResult(direction, node, res, x.freeze());
	}
}
