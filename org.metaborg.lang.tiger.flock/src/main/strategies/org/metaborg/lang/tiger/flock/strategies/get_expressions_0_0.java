...
public class get_expressions_0_0 extends Strategy {
	...
	public IStrategoTerm invoke(Context context, IStrategoTerm current) {
		...
		Flock.instance.analysisWithName("expressions").performDataAnalysis(Flock.instance.graph,
				Flock.instance.graph_scss, node);
		IStrategoList result = factory.makeList(
				((Map<IStrategoTerm, IStrategoTerm>) node.getProperty("expressions").lattice.value()).entrySet()
						.stream().map(e -> factory.makeList(Helpers.toTerm(e.getKey()), Helpers.toTerm(e.getValue())))
