package org.metaborg.lang.tiger.flock.experiments.fastcapsulevalue;
...
import org.metaborg.lang.tiger.flock.common.FlockLattice.ImmutableMap;
...
import org.metaborg.lang.tiger.flock.experiments.fastcapsulevalue.FastCapsuleValueAnalysisProperties.ConstProp;
import org.metaborg.lang.tiger.flock.experiments.fastcapsulevalue.FastCapsuleValueAnalysisProperties.ConstPropBottom;
import org.metaborg.lang.tiger.flock.experiments.fastcapsulevalue.FastCapsuleValueAnalysisProperties.ConstPropConst;
import org.metaborg.lang.tiger.flock.experiments.fastcapsulevalue.FastCapsuleValueAnalysisProperties.ConstPropTop;
...
public class FastCapsuleValueAnalysis extends SingleAnalysis {
	public FastCapsuleValueAnalysis(int id) {
		super("values", Direction.FORWARD, id);
	}

	@Override
	public void initNodeValue(Node node) {
		node.addProperty("values", ImmutableMap.bottom());
	}
...
class Value implements FlockValueLattice {
	...
	public boolean lub(Object other) {
		...
		if (TermUtils.isTuple(term4)
			...
			result10 = (boolean) usri.equals(usrj) ? new ConstPropConst(Integer.parseInt(M.string(usri)))
					: new ConstPropTop();
		}
		if (TermUtils.isTuple(term4)
...
class Start extends TransferFunction {
	...
	@Override
	public boolean eval(SingleAnalysis.Direction direction, FlockLattice res, Node node) {
		ImmutableMap tmp43 = (ImmutableMap) ImmutableMap.bottom();
		boolean r = TransferFunction.assignEvalResult(direction, node, res, tmp43);
		return r;
	}
}
...
class TransferFunction5 extends TransferFunction {
	...
	public boolean eval(SingleAnalysis.Direction direction, FlockLattice res, Node node) {
		...
		Transient x = ((io.usethesource.capsule.Map.Immutable) ((FlockLattice) UserFunctions.values_f(prev)).value())
				.asTransient();
		x.__put(usrn, new Value(new ConstPropConst(Integer.parseInt(M.string(usri)))));
